(function() {
  ...
  function openSearch() {
    ...
    // Delay focus for smooth animation
  ...
  function loadSearchData() {
    const source = searchModal.getAttribute('data-search-source') || '/search.json';
    fetch(source)
  ...
  function performSearch() {
    ...
    if (!isDataLoaded) {
      const loadingText = searchModal.getAttribute('data-i18n-loading') || 'Loading...';
      searchResults.innerHTML = `<div class="search-loading">${loadingText}</div>`;
      return;
    }
  ...
  function showEmptyState() {
    const text = searchModal.getAttribute('data-i18n-empty') || 'Type to search articles...';
    searchResults.innerHTML = `<div class="search-empty-state">${text}</div>`;
  }
  ...
  function displayResults(results, query) {
    ...
    if (results.length === 0) {
      const noResultsText = searchModal.getAttribute('data-i18n-no-results') || 'No results found';
    ...
    // Bind hover events for each result
